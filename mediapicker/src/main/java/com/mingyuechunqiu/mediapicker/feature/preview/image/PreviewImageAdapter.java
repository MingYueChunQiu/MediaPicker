 ...
 *     time   : 2019/4/25
 *     desc   : 预览图片列表适配器
 *              继承自MediaPickerBaseAdapter
 *     version: 1.0
 * </pre>
 */
class PreviewImageAdapter extends BaseMediaPickerAdapter<MediaAdapterItem, BaseViewHolder> {

    PreviewImageAdapter(int layoutResId, @Nullable List<MediaAdapterItem> data, int maxSelectCount, long limitSize, long limitDuration, OnItemSelectChangedListener listener) {
    ...
    protected void convert(BaseViewHolder helper, final MediaAdapterItem item) {
        AppCompatImageView ivShow = helper.getView(R.id.iv_mp_preview_image_item_show);
        if (ivShow != null) {
        ...
        helper.setChecked(R.id.cb_mp_preview_image_item_checked, item.isChecked())
                .setOnCheckedChangeListener(R.id.cb_mp_preview_image_item_checked, new CompoundButton.OnCheckedChangeListener() {
                    @Override
